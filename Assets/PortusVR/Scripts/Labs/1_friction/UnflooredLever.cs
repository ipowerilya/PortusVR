...
namespace BNG
{
    ...
    /// </summary>
    public class UnflooredLever : MonoBehaviour
    {
        ...
        public virtual float GetAnglePercentage(float currentAngle)
        {
            if (hingedJoint)
            {
                return (currentAngle - hingedJoint.limits.min) * 100f / (hingedJoint.limits.max - hingedJoint.limits.min);
            }

            if (configJoint)
            {
                return currentAngle * 100f / configJoint.linearLimit.limit;
            }
